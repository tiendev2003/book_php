                    ...
                    <div class="card">
                        <div class="card-header">
							...
							<form action="?route=order" method="post">
								<select name="statusorderid" onchange="this.form.submit()">
								<option <?php if($statusorderid == 0): ?>selected<?php endif ?> value="0">Tất cả </option>
								<?php foreach ($liststatus as $status): ?>
									<option <?php if($status->getStatusOrderId() == $statusorderid): ?>selected<?php endif ?> value="<?php echo $status->getStatusOrderId() ?>"><?php echo $status->getStatusOrderName() ?></option>
								<?php endforeach ?>
								</select>
							</form>
                                ...
                                <tbody class="table-border-bottom-0">
                                    ...
                                    <tr>
                                        ...
                                        <td><a type="button"
                                            ...
                                            <?php if($order->getStatusOrder()->getStatusOrderId() == 1 || $order->getStatusOrder()->getStatusOrderId() == 2): ?>
                                            <a id="editButton" type="button"
                                                    class="btn rounded-pill btn-warning"
                                                    href="?route=transition&userid=<?php echo $order->getUser()->getUserId() ?>&orderid=<?php echo $order->getOrderId() ?>">Chuyển</a>
                                            <?php endif ?>
                                        </td>
                            ...
                            <nav aria-label="Page navigation">
                                <ul class="pagination">
                                    <?php if ($currentPage != 0): ?>
                                        <li class="page-item">
                                            <a href="?route=order&page=<?php echo $currentPage - 1; ?>" class="page-link">Trước</a>
                                        </li>
                                    <?php endif; ?>
                                    
                                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                        <li class="page-item <?php echo ($currentPage == $i - 1) ? 'active' : ''; ?>">
                                            <a href="?route=order&page=<?php echo $i - 1; ?>" class="page-link"><?php echo $i; ?></a>
                                        </li>
                                    <?php endfor; ?>
                                    
                                    <?php if ($currentPage + 1 != $totalPages): ?>
                                        <li class="page-item">
                                            <a href="?route=order&page=<?php echo $currentPage + 1; ?>" class="page-link">Sau</a>
                                        </li>
                                    <?php endif; ?>
                                </ul>
                            </nav>
